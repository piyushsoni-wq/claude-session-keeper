...
// Writes the <session-id>/custom-title.json side-car this repo already
// reads with top priority (see analyzeSessionFile / readCustomTitleSidecar
// above) — renaming through the dashboard uses the exact same mechanism
// Claude Code itself uses, rather than inventing a second one.
function writeSessionTitle(rootDir, sessionId, title) {
  ...
  fs.writeFileSync(path.join(sessionDir, 'custom-title.json'), JSON.stringify({ customTitle: trimmed }), 'utf8');
  return trimmed;
}
